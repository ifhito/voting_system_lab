...
async function overWriteDataToDB(databaseObj, docPath, newData){
  ...
  databaseObj.doc(docPath).update(
...
async function getDataFromDB(databaseObj, docPath){
  ...
  var tmp = await docRef.get().then(function(doc) {
      if (doc.exists) {
...
async function getVoteData(databaseObj, timestamp){
  var data = await getDataFromDB(databaseObj, "vote_data/"+timestamp.toString());
  return data;
};
...
function updatePresentationDuration(databaseObj, dateTimestamp, presenterID, duration){
